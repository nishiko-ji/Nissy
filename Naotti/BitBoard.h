...
struct BitBoardN
{
	...
	int makeMoves(int teban, int kiki[], int from[], int to[])
	{
		...
		{
			if (teban == 0 && !(((existR | existB) >> pos) & 1))
				continue;
			if (teban == 1 && !((existP >> pos) & 1))
				continue;
			for (i = pos * 5; kiki[i] != -1; i++)
			{
				int npos = kiki[i];
				//マスpos -> マスnposと駒を動かせるか？（自駒とぶつからないか）
				if (teban == 0 && (((existR | existB) >> npos) & 1))
					continue;
				if (teban == 1 && ((existP >> npos) & 1))
					continue;
				//動かせる
				from[cnt] = pos;
				to[cnt] = npos;
	...
	int getWinPlayer(int teban, int pnum)
	{
		if (existR == 0)
			return 0;
		if (existB == 0 || bb::bitCount(existP) <= pnum)
			return 1;
		if (teban == 0 && ((existB & 1LL) || ((existB >> 5) & 1LL)))
			return 0;
		if (teban == 1 && (((existP >> 30) & 1LL) || ((existP >> 35) & 1LL)))
			return 1;
		return 2;
	}
	...
	MoveCommandN getEscapeCommand(int teban)
	{
		if (teban == 0)
		{
			if (existB & 1LL)
				return MoveCommandN(0, 0, 3);
			if ((existB >> 5) & 1LL)
				return MoveCommandN(0, 5, 1);
			return MoveCommandN(-1, -1, -1);
		}
		else
		{
			if ((existP >> 30) & 1LL)
				return MoveCommandN(5, 0, 3);
			if ((existP >> 35) & 1LL)
				return MoveCommandN(5, 5, 1);
			return MoveCommandN(-1, -1, -1);
		}
	}
